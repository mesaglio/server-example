...
def test_get_empty_users(test_client: FlaskClient):
    response = test_client.get('/usuarios')
    assert response.status_code == 200
    assert b'[]' in response.data


def test_create_user(test_client: FlaskClient, user: dict):
    response = test_client.post('/usuarios', json=user)
    assert response.status_code == 201


def test_create_user_bad_request(test_client: FlaskClient, user: dict):
    del user['documento']
    print(user)
    response = test_client.post('/usuarios', json=user)
...
def test_get_created_user(test_client: FlaskClient, user: dict):
    response = test_client.post('/usuarios', json=user)
    assert response.status_code == 201
    response2 = test_client.get(f"/usuarios/{user.get('username')}")
    assert response2.status_code == 200
    assert user.get('username').encode() in response2.data
    assert user.get('nombres').encode() in response2.data
    assert user.get('apellidos').encode() in response2.data
    assert user.get('genero').encode() in response2.data
    assert user.get('documento').encode() in response2.data
    assert user.get('fechaNacimiento').encode() in response2.data
...
def test_not_found_user(test_client: FlaskClient):
